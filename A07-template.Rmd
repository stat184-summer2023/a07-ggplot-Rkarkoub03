...
Trips$duration<-as.numeric(Trips$edate-Trips$sdate)

lower=quantile(Trips$duration,0.25) - 1.25*(quantile(Trips$duration,.75)-quantile(Trips$duration,0.25))

upper=quantile(Trips$duration,0.75) + 1.25*(quantile(Trips$duration,.75)-quantile(Trips$duration,0.25))

new_trips<-subset(Trips, duration>=lower & duration<=upper)

ggplot(new_trips,aes(x=client,y=duration))+geom_boxplot()+labs(x="Client",y="Duration(seconds)")
...
Trips %>%
  ...
  geom_histogram( color = "black", bins = 365) +
  labs(x = "Day of the Year", y = "Frequency") +
  ggtitle("Bike Use by Day of the Year")

```


the different peaks of the histogram shows higher bike usage in the warmer seasons than there is in the colder seasons. 

### Day of Week

```{r}
Trips %>%ggplot(aes(x = lubridate::wday(sdate, label = TRUE))) +
...
The plot shows that the weekends tend to have less people using their bikes.
...
```{r}
Trips %>%ggplot(aes(x = lubridate::hour(sdate))) +
...
The ploy shows that the bike usage is lower during the earliest and latest parts of the day
...
```{r}
Trips %>% ggplot(aes(x = lubridate::minute(sdate))) +
  geom_histogram( color = "black", bins = 40
      ) +
  labs(x = "Minute in the Hour", y = "Frequency") +
  ggtitle("Bike Use by Minute in the Hour")

```
There doesn't seem to be a clear trend showing based off of this plot
...
```{r}
Trips %>%
  group_by(hour = lubridate::hour(sdate),
           day_of_week = lubridate::wday(sdate, label = TRUE),
           client_type) %>%
  summarise(total_events = n()) %>%
  ggplot(aes(x = hour, y = total_events, fill = client_type)) +
  geom_col(position = "dodge", width = 0.8) +
  facet_grid(day_of_week ~ .) +
  labs(x = "Hour of the Day", y = "Total Events", fill = "Client Type") +
  ggtitle("Bike Rentals by Hour, Day of the Week, and Client Type")
```
...
```{r}
Tripsdist<-inner_join(Trips,PairDistances,by=c("sstation","estation"))

ggplot()+geom_density(data=Tripsdist,aes(x=distance,color="Ride Distance",))+geom_density(data=PairDistances,aes(x=distance,color="Station Pair Distance"))+labs(x="Distance",y="Density")
